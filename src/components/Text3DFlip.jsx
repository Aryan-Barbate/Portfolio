...
export function Text3DFlip({
  ...
  return (
    <ElementTag
      className={`text-3d-flip-root ${className ?? ""}`}
    ...
    >
      ...
      {characters.map((wordObj, wordIndex) => (
        <span key={wordIndex} className="text-3d-flip-word">
          ...
          {wordObj.needsSpace && <span className="text-3d-flip-space"> </span>}
        </span>
      ))}
    </ElementTag>
  )
...
const CharBox = memo(({ char, textClassName, flipTextClassName, rotateDirection }) => (
  ...
  >
    <span
      className={`text-3d-flip-front ${textClassName ?? ""}`}
      style={{ transform: FRONT_FACE_TRANSFORMS[rotateDirection] }}
    >
      {char}
    </span>
    <span
      className={`text-3d-flip-back ${flipTextClassName ?? ""}`}
